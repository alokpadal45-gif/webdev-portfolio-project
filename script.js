...
// nepal time (UTC+5:45) needs manual calculation
...
function tickClocks() {
  ...
  if (nepalDateEl) {
    nepalDateEl.textContent =
      np.toLocaleDateString('en-US', { weekday:'short', month:'short', day:'numeric', year:'numeric' });
  }

  const nepalModeEl = document.getElementById('nepalMode');
  if (nepalModeEl) {
    nepalModeEl.textContent =
